...
function encodeToRot13(str) {
  const gap = 13;
  const uppercaseArr = 'ABCDEFGHIJKLMNOPQRSTUVWXYZABCDEFGHIJKLM'.split('');
  const lowercaseArr = 'abcdefghijklmnopqrstuvwxyzabcdefghijklm'.split('');
  return str.split('').map((el) => {
    if (uppercaseArr.includes(el)) return uppercaseArr[uppercaseArr.indexOf(el) + gap];
    if (lowercaseArr.includes(el)) return lowercaseArr[lowercaseArr.indexOf(el) + gap];
    return el;
  }).join('');
}
